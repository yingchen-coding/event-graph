...
TITLE = 'event-graph — your logs, as a queryable graph'
LINES = [
    ('$ event-graph convert-macos-log --input log.json --output events.csv', 'cmd'),
    ('$ event-graph --db g.duckdb ingest --events events.csv', 'cmd'),
    ('  ingested 100,000 events  →  processes · files · sockets · agents', 'ok'),
    ('', 'blank'),
    ('$ event-graph --db g.duckdb related --seed process:curl --hops 2', 'cmd'),
    ('  curl → wrote /tmp/x.sh → bash ran it → connected 4 external IPs', 'err'),
    ('', 'blank'),
    ('$ event-graph --db g.duckdb malware-hits', 'cmd'),
    ('  3 pipe-to-shell · 1 reverse-shell · 0 false positives', 'ok'),
    ('  # one query surfaces the whole chain, not a single alert', 'comment'),
]
